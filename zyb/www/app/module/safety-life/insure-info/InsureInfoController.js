 ...
 */
angular.module('app.controllers').controller('InsureInfoController', function ($scope, $state, $ionicLoading, CheckService, InsureInfoService) {
    var price = 0; // 每份单价

    var init = function () {
        price = parseFloat(InsureInfoService.getSelectedProduct().desc);
    ...
    $scope.insure = function () {
        ...
        if (message) {
            ...
            InsureInfoService.insure($scope.policyHolder, $scope.insured, $scope.address, $scope.insurancePeriod, $scope.baseInfo).then(function () {
                $state.go('pay');
            }).catch(function (err) {
                $ionicLoading.show({
                    template: '投保失败',
