 ...
 */
angular.module('app.controllers').controller('goldProtectInsureInfoController', function ($scope, $state, $ionicLoading, CheckService, goldProtectInsureInfoService) {
    var price = 0; // 每份单价

    var init = function () {
        price = parseFloat(goldProtectInsureInfoService.getSelectedProduct().desc);
    ...
    $scope.insure = function () {
        ...
        if (message) {
            ...
            goldProtectInsureInfoService.insure($scope.policyHolder, $scope.insured, $scope.address, $scope.insurancePeriod, $scope.baseInfo).then(function () {
                $state.go('pay');
            }).catch(function (err) {
                $ionicLoading.show({
                    template: '投保失败',
